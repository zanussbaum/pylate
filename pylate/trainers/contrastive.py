...
class SingleDatasetBatchSampler(BatchSampler):
    ...
    def __iter__(self) -> Iterator[List[int]]:
        while True:
            ...
            print(f"Selected dataset {dataset_idx}")
            # print_in_order(f"Selected dataset {dataset_idx}")
            ...
            if current_pos + self.global_batch_size > len(dataset_indices):
                ...
                self.indices_per_dataset[dataset_idx] = torch.randperm(
                    self.dataset_sizes[dataset_idx],
    ...
    def batch_size(self) -> int:
        return self.global_batch_size // dist.get_world_size()
